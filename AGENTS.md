...
3. **Put logic in `src/` (the library), not in `src/bin/`.** The library is
   ...
   regression this rule was written for was selection logic that ended up in a
   binary and so had no test to mutate.
...
- **`top` derived from the ground-truth width.** A search config without `top`
  takes it from the dataset's neighbour count. On a wide-ground-truth dataset
  that runs every point at k=width, and HNSW raises effective breadth to at least
  k — so an `ef` sweep publishes one measurement under several config names.
...
Most entries in `datasets/datasets.json` auto-download from their `link`.
