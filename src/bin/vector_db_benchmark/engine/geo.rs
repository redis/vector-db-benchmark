...
//! so the absolute error in the *implied* radius is `≈ 1.1e-16 / (r/R²)`: about
//! 0.2 µm at the shipped radii (~3·10⁵ m) and still under 5 mm at r = 1 m. The
//! dominant error is not this but the choice of earth radius.

/// Mean earth radius, metres.
///
/// Chosen to match `tests/common/mod.rs::haversine_m`, which brute-forces the
/// geo fixtures' ground truth. The reference engines disagree here by up to
/// ~0.1 % (RediSearch's geohash library uses 6 372 797.56 m, PostgreSQL's
/// `earthdistance` uses 6 378 168 m, Elasticsearch ~6 371 008.8 m), so a
/// boundary point can land on either side depending on the engine. Every geo
/// fixture in this repo keeps its documents clear of the boundary by a margin
/// far larger than that spread, and the shipped datasets' ground truth was
/// generated by the upstream haversine generator on the same mean radius.
...
///
/// Literals go out with `{:?}` (Rust's shortest round-tripping form) so the
/// server parses back exactly the `f64` computed here; `{}` would print
/// `0.30000000000000004` as-is but truncates nothing only by accident.
pub fn cap_expression(
    ...
    let terms: Vec<String> = COMPONENTS
        .iter()
        .zip(q.iter())
        .map(|(axis, coeff)| format!("{} * {:?}", sel(&component_field(field, axis)), coeff))
        .collect();
    Some(format!("({}) >= {:?}", terms.join(" + "), cos_theta))
}
...
mod tests {
    ...
    fn rendered_literals_round_trip_exactly() {
        let criteria = json!({"lon": 116.0, "lat": -52.0, "radius": 326_341.0});
        ...
        for v in q.iter().chain(std::iter::once(&cos_theta)) {
            assert!(e.contains(&format!("{v:?}")), "{v:?} not verbatim in {e}");
        }
    }
}
