...
- **Multi-valued `labels` arrays** — a KiviDB TAG value is *atomic*, never split on a separator (`FT.CREATE` rejects the `SEPARATOR` modifier, and `@f:{b}` does not match a stored `a;b;c`), so `match_any` over a labels field could only ever match the whole joined string. Rejected at upload time when the field is declared in the dataset schema.
...
- **TAG values containing `|` or `@`** — inexpressible with no escaping mechanism (`|` is always the TAG-OR separator; `@` degrades the whole query to match-all).
